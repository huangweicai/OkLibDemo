package com.oklib.view.tabsegment;
...
public class TabSegment extends View {
    ...
    public TabSegment(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray ta = context.obtainStyledAttributes(attrs,
                R.styleable.oklib_tabsegment);

        String textArray = ta.getString(R.styleable.oklib_tabsegment_oklib_texts);
        if (textArray != null) {
            mTexts = textArray.split("\\|");
        }

        mTextSize = ta.getDimensionPixelSize(
                R.styleable.oklib_tabsegment_android_textSize, (int) TypedValue
                        .applyDimension(TypedValue.COMPLEX_UNIT_SP, 14, context
                                .getResources().getDisplayMetrics()));
        mCornerRadius = ta.getDimensionPixelSize(
                R.styleable.oklib_tabsegment_oklib_cornerRadius, (int) TypedValue
                        .applyDimension(TypedValue.COMPLEX_UNIT_DIP, 5, context
                                .getResources().getDisplayMetrics()));
        mDirection = Direction.values()[ta.getInt(
                R.styleable.oklib_tabsegment_oklib_direction, 0)];

        mHorizonGap = ta.getDimensionPixelSize(
                R.styleable.oklib_tabsegment_oklib_horizonGap, 0);
        mVerticalGap = ta.getDimensionPixelSize(
                R.styleable.oklib_tabsegment_oklib_verticalGap, 0);

        int gap = ta.getDimensionPixelSize(R.styleable.oklib_tabsegment_oklib_gaps,
        ...
        mColors = ta.getColorStateList(R.styleable.oklib_tabsegment_oklib_colors);
        mSelectedTextColor = ta.getColor(
                R.styleable.oklib_tabsegment_oklib_textSelectedColors, Color.WHITE);
        if (mColors == null) {
            mColors = new ColorStateList(new int[][]{{}},
                    new int[]{0xFFF4CC66});
        }

        mBoundWidth = ta.getDimensionPixelSize(
                R.styleable.oklib_tabsegment_oklib_boundWidth, mBoundWidth);
        mSeparatorWidth = ta.getDimensionPixelSize(
                R.styleable.oklib_tabsegment_oklib_separatorWidth, mSeparatorWidth);
    ...
    public void setTextSize(int unit, int textSize) {
        mPaint.setTextSize((int) (TypedValue.applyDimension(unit, textSize,
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mTexts != null && mTexts.length > 0) {
            ...
            switch (widthMode) {
                case MeasureSpec.AT_MOST:
                    if (mDirection == Direction.HORIZONTAL) {
                        ...
                        width = widthSize <= mSingleChildWidth ? widthSize
                                : mSingleChildWidth;
                    }
                    break;
            ...
            switch (heightMode) {
                case MeasureSpec.AT_MOST:
                    if (mDirection == Direction.VERTICAL) {
                        ...
                        height = heightSize <= mSingleChildHeight ? heightSize
                                : mSingleChildHeight;
                    }
                    break;
                case MeasureSpec.EXACTLY:
            ...
            for (int i = 0; i < mTexts.length; i++) {
                ...
                mCacheBounds[i].right = mCacheBounds[i].left
                        + mSingleChildWidth;
                mCacheBounds[i].bottom = mCacheBounds[i].top
                        + mSingleChildHeight;
            }
        } else {
        ...
        if (mTexts != null && mTexts.length > 0) {
            ...
            for (int i = 0; i < mTexts.length; i++) {
                ...
                if (i < mTexts.length - 1) {
                    ...
                    if (mDirection == Direction.HORIZONTAL) {
                        canvas.drawLine(mCacheBounds[i].right, 0,
                                mCacheBounds[i].right, getHeight(), mPaint);
                    } else {
                        canvas.drawLine(mCacheBounds[i].left,
                                mSingleChildHeight * (i + 1),
                                mCacheBounds[i].right, mSingleChildHeight
                                        * (i + 1), mPaint);
                    }
                }
                ...
                if (i == mCurrentIndex && mSelectedDrawable != null) {
                    ...
                    mSelectedDrawable.setRadius(topLeftRadius, topRightRadius,
                ...
                float baseline = mCacheBounds[i].top
                        + ((mSingleChildHeight - mCachedFM.ascent + mCachedFM.descent) / 2)
                        - mCachedFM.descent;
                canvas.drawText(mTexts[i], mCacheBounds[i].left
                                + (mSingleChildWidth - mTextBounds[i].width()) / 2,
                        baseline, mPaint);
    ...
    public void setOnSegmentControlClickListener(
            OnSegmentControlClickListener listener) {
