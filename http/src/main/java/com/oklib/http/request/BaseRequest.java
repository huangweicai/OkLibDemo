...
import com.oklib.http.adapter.CacheCall;
import com.oklib.http.adapter.Call;
import com.oklib.http.adapter.CallAdapter;
import com.oklib.http.adapter.DefaultCallAdapter;
import com.oklib.http.cache.CacheEntity;
import com.oklib.http.cache.CacheMode;
import com.oklib.http.callback.AbsCallback;
import com.oklib.http.convert.Converter;
...
public abstract class BaseRequest<R extends BaseRequest> {
    ...
    public BaseRequest(String url) {
        ...
        OklibHttp go = OklibHttp.getInstance();
    ...
    public okhttp3.Call generateCall(Request request) {
        ...
        if (readTimeOut <= 0 && writeTimeOut <= 0 && connectTimeout <= 0 && sslParams == null && userCookies.size() == 0) {
            return OklibHttp.getInstance().getOkHttpClient().newCall(request);
        } else {
            OkHttpClient.Builder newClientBuilder = OklibHttp.getInstance().getOkHttpClient().newBuilder();
            ...
            if (userCookies.size() > 0) OklibHttp.getInstance().getCookieJar().addCookies(userCookies);
