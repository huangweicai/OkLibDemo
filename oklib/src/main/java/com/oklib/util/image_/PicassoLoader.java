...
public class PicassoLoader implements ILoaderProxy {
    ...
    private static LruCache sLruCache = new LruCache(OkLib.appContext);
    ...
    private static Picasso getPicasso() {
        if (sPicassoSingleton == null) {
            synchronized (PicassoLoader.class) {
                if (sPicassoSingleton == null) {
                    sPicassoSingleton = new Picasso.Builder(OkLib.appContext).memoryCache(sLruCache).build();
                }
            }
        }
    ...
    public void saveImage(String url, final File destFile, final Callback callback) {
        getPicasso().load(url).into(new Target() {
            ...
            public void onBitmapLoaded(Bitmap bitmap, Picasso.LoadedFrom from) {
                ...
                OkLib.appContext.sendBroadcast(new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE, Uri.fromFile(destFile)));
...
//            }
    ...
    public void clearDiskCache() {
        File diskFile = new File(OkLib.appContext.getCacheDir(), PICASSO_CACHE);
        if (diskFile.exists()) {
