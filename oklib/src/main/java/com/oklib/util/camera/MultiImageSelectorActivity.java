...
public class MultiImageSelectorActivity extends FragmentActivity implements MultiImageSelectorFragment.Callback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.oklib_activity_default);
        setStateBarStyle();
        ...
        if (resultList == null || resultList.size() <= 0) {
            mSubmitButton.setText(R.string.oklib_action_done);
...
//                            | View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR;//状态栏字体深灰色（6.0以上属性）（注意：1.以最后一个设置为准 2.特定的颜色值才有效果）
    ...
    private void updateDoneText() {
        mSubmitButton.setText(String.format("%s(%d/%d)",
                getString(R.string.oklib_action_done), resultList.size(), mDefaultCount));
    }
