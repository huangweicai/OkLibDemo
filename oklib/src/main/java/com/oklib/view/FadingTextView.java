...
import java.lang.annotation.RetentionPolicy;
 ...
 /**
   ...
   * 描述：一个可以让TextView以淡如淡出的形式不断改变自己内容
   */
public class FadingTextView extends android.support.v7.widget.AppCompatTextView {
    ...
    private CharSequence[] texts = new CharSequence[]{};
     ...
     */
    private void init() {
        fadeInAnimation = AnimationUtils.loadAnimation(getContext(), R.anim.oklib_fadein);
        fadeOutAnimation = AnimationUtils.loadAnimation(getContext(), R.anim.oklib_fadeout);
        handler = new Handler();
        isShown = true;
    }
    ...
    private void handleAttrs(AttributeSet attrs) {
        TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.oklib_fading_text_view);
        this.texts = a.getTextArray(R.styleable.oklib_fading_text_view_oklib_fading_texts);
        this.timeout = Math.abs(a.getInteger(R.styleable.oklib_fading_text_view_oklib_fading_timeout, 14500)) + getResources().getInteger(android.R.integer.config_longAnimTime);
        a.recycle();
    }
