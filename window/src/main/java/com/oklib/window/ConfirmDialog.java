...
public class ConfirmDialog extends BaseDialogFragment implements View.OnClickListener {
     ...
     * 设置内容位置
     */
    public void setContentGravity(final int gravity) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                while (null == tv_content) {
                ...
                getActivity().runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        //设置控件相对外位置
//                        LinearLayout.LayoutParams llParams = (LinearLayout.LayoutParams) tv_content.getLayoutParams();
//                        llParams.gravity = gravity;
//                        tv_content.setLayoutParams(llParams);
